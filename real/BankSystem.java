...
public class BankSystem extends JFrame implements ActionListener, ItemListener {
	...
	private JMenuItem addNew, update,NewEmployee,UpdateEmployee,printRec, end;				//File Menu Options.
	private	JMenuItem  deposit, withdraw, MoneyTransfer,FD,DD, delRec, search, searchName;	//Edit Menu Options.
	private	JMenuItem oneByOne, allCustomer,BankBranches2,Employees2,Departments2;				//View Menu Options.
	...
	public BankSystem () {
		...
		update = new JMenuItem ("Update Account", new ImageIcon ("Images/Open.gif"));
		update.setAccelerator (KeyStroke.getKeyStroke(KeyEvent.VK_U, Event.CTRL_MASK));
		update.setMnemonic ((int)'U');
		update.addActionListener (this);
		
		
		NewEmployee = new JMenuItem ("Create New Employee", new ImageIcon ("Images/Open.gif"));
		NewEmployee.setAccelerator (KeyStroke.getKeyStroke(KeyEvent.VK_L, Event.CTRL_MASK));
		NewEmployee.setMnemonic ((int)'N');
		NewEmployee.addActionListener (this);
		
		UpdateEmployee = new JMenuItem ("Update Employee", new ImageIcon ("Images/Open.gif"));
		UpdateEmployee.setAccelerator (KeyStroke.getKeyStroke(KeyEvent.VK_Y, Event.CTRL_MASK));
		UpdateEmployee.setMnemonic ((int)'U');
		...
		/*MoneyTransfer = new JMenuItem ("Transfer Money");
		MoneyTransfer.setAccelerator (KeyStroke.getKeyStroke(KeyEvent.VK_M, Event.CTRL_MASK));
		MoneyTransfer.setMnemonic ((int)'M');	
		MoneyTransfer.addActionListener (this);
		
		DD = new JMenuItem ("Draft");
		DD.setAccelerator (KeyStroke.getKeyStroke(KeyEvent.VK_F, Event.CTRL_MASK));
		DD.setMnemonic ((int)'F');	
		DD.addActionListener (this);
		
		FD = new JMenuItem ("Fixed Deposit");
		FD.setAccelerator (KeyStroke.getKeyStroke(KeyEvent.VK_X, Event.CTRL_MASK));
		FD.setMnemonic ((int)'F');	
		...
		*/
		...
		BankBranches2 = new JMenuItem ("Bank Branch");
		BankBranches2.setAccelerator (KeyStroke.getKeyStroke(KeyEvent.VK_B, Event.CTRL_MASK));
		BankBranches2.setMnemonic ((int)'I');	
		BankBranches2.addActionListener (this);
		Employees2 = new JMenuItem ("View Employees", new ImageIcon ("Images/refresh.gif"));
		Employees2.setAccelerator (KeyStroke.getKeyStroke(KeyEvent.VK_V, Event.CTRL_MASK));
		Employees2.setMnemonic ((int)'J');
		Employees2.addActionListener (this);
		Departments2 = new JMenuItem ("View Departments", new ImageIcon ("Images/refresh.gif"));
		Departments2.setAccelerator (KeyStroke.getKeyStroke(KeyEvent.VK_V, Event.CTRL_MASK));
		Departments2.setMnemonic ((int)'P');
		Departments2.addActionListener (this);
		...
		mnuFile.add (update);
		mnuFile.addSeparator ();
		mnuFile.add (NewEmployee);
		mnuFile.addSeparator ();
		mnuFile.add (UpdateEmployee);
		mnuFile.addSeparator ();
		...
		*/
		...
		mnuView.addSeparator ();
		mnuView.add (BankBranches2);
		mnuView.addSeparator ();
		mnuView.add (Employees2);
		mnuView.addSeparator ();
		mnuView.add (Departments2);
	...
	public void actionPerformed (ActionEvent ae) {
		...
		if (obj == NewEmployee || obj == open || obj == btnNew) {

			boolean b = openChildWindow ("Create New Empoyee");
			if (b == false) {
				NewEmployee newAcc = new NewEmployee ();
				desktop.add (newAcc);
				newAcc.show ();
			}
		...
		if (obj == update || obj == open || obj == btnNew) {

			boolean b = openChildWindow ("Update Account");
			if (b == false) {
		...
		if (obj == UpdateEmployee || obj == open || obj == btnNew) {

			boolean b = openChildWindow ("Update Employee");
			if (b == false) {
				UpdateEmployee upAcc = new UpdateEmployee ();
				desktop.add (upAcc);
				upAcc.show ();
			}
		...
		/*else if (obj == MoneyTransfer || obj == with || obj == btnWith) {

			boolean b = openChildWindow ("Money Transfer");
			if (b == false) {
				MoneyTransfer MonTran = new MoneyTransfer ();
				desktop.add (MonTran);
				MonTran.show ();
			}
		...
		else if (obj == FD || obj == with || obj == btnWith) {

			boolean b = openChildWindow ("FD");
			if (b == false) {
		...
		else if (obj == DD || obj == with || obj == btnWith) {
		...
		else if (obj == Departments2 || obj == all) {

			boolean b = openChildWindow ("View All Departments");
			if (b == false) {
				Departments viewCus = new Departments ();
				desktop.add (viewCus);
				viewCus.show ();
			}
		...
		else if (obj == Employees2 || obj == all) {

			boolean b = openChildWindow ("View All Account Holders");
			if (b == false) {
				Employees viewCus = new Employees ();
				desktop.add (viewCus);
				viewCus.show ();
			}
		...
		else if (obj == BankBranches2 || obj == all) {

			boolean b = openChildWindow ("View All Account Holders");
			if (b == false) {
				BankBranches viewCus = new BankBranches ();
				desktop.add (viewCus);
				viewCus.show ();
			}
			...
			for(int getFrameLoop = 0; getFrameLoop < Frames.length; getFrameLoop++) {
				...
				String msg = "BankSystem [Pvt] Limited.\n\n" + "Menu Items:- File,  Edit,  View,  Options,  Window,  Help\n\n" + 
				"File:-Open New Account,  Update Account,  Print Balance,  Quit\n\n Edit:-  Deposit,  Withdraw,  Transfer,  FD,  Draft,  Delete,  Search" + "\n\nView:-  OneByOne,  All Customers,  BankBranches,  Employees,  Departments\n\nOptions:-Change Background,  Layout,  Theme\n\nWindow:-CloseAllActiveWindows,  CloseAllWindows\n\nHelp:-HelpContents,  ShortcutHelp,  About\n\n";
			JOptionPane.showMessageDialog (this, msg, "About BankSystem", JOptionPane.PLAIN_MESSAGE);

		}
		else if (obj == keyHelp || obj == btnKey) {

				String msg = "BankSystem [Pvt] Limited.\n\n" + "File:-\nctrl+n=Open New Account\nctrl+u=Update Account\n  ctrl+r=Print Balance\n ctrl+q=Quit\n\n Edit:-\nctrl+t=Deposit\nctrl+w=Withdraw\nctrl+m=Transfer\nctrl+x=FD\nctrl+f=Draft\nctrl+d=Delete\nctrl+s or m=Search" + "\n\nView:-\nctrl+o=OneByOne\nctrl+a=All Customers\nctrl+b=BankBranches\nctrl+v=Employees\n  ctrl+v=Departments\n\nOptions:-\nctrl+b=Change Background\n\nHelp:-\nctrl+h=HelpContents\nctrl+k=ShortcutHelp\nctrl+c=About";
			JOptionPane.showMessageDialog (this, msg, "About BankSystem", JOptionPane.PLAIN_MESSAGE);
			...
			String msg = "BankSystem [Pvt] Limited.\n\n" + "Created & Designed By:\n" + 
				"Rachit Verma And Jitendra Shukla\n\n" + "E-mail me:\n rachitverma.001@gmail.com";
			JOptionPane.showMessageDialog (this, msg, "About BankSystem", JOptionPane.PLAIN_MESSAGE);
